...
$(window).on("ready", function(event){
	...
	var addCart = function(event){
		...
		var sWidth = $this.data("width");
		var sHeight = $this.data("height");
		var sSquare = $this.data("square");
				...
				if($this.hasClass("multi")){
					...
					if(parseInt(productID, 10) > 0){
						...
						$.getJSON(ajaxPath, gObj).done(function(jData){
							...
							if( _urlPath.match( /^\/catalog\/fotooboi-i-freski\/[\D\d]*?\.html$/g ) !== null ) {
								setTimeout( () => {
									$this.removeClass("added")
									.removeClass("bwOpened")
									.prepend( $imageAfterLoad.attr( "src", TEMPLATE_PATH + "/images/incart.png" ) )
									.html( 'В корзину' )
									.attr( 'href', '#' );
								}, 1500 );
							}
